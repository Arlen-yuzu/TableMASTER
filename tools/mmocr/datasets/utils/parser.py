...
def get_bbox_nums_by_text(text):
    text = text.split(',')
    # xyl add th
    pattern = ['<td></td>', '<td', '<th></th>', '<th', '<eb></eb>',
...
def align_bbox_mask(bboxes, empty_bbox_mask, label):
    ...
    pattern = ['<td></td>', '<td', '<eb></eb>',
               ...
               '<eb7></eb7>', '<eb8></eb8>', '<eb9></eb9>',
               '<eb10></eb10>']
    assert len(bboxes) == get_bbox_nums_by_text(label) == len(empty_bbox_mask), print(len(bboxes), get_bbox_nums_by_text(label), len(empty_bbox_mask))
    bbox_count = 0
