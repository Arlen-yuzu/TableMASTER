...
class Evaluator():
    ...
    def __init__(self, cal_tasks=None, text_match_align=False, align_match_text=False):

        self.cal_tasks = cal_tasks  # ['acd', 'ctc', 'tsr_rc', 'tsr_teds', 'tsr_ll']

        self.t2a = text_match_align

        self.a2t = align_match_text
        ...
        self.BBD_allt = 0  # R = tp / allt     ground turth results
    ...
    def correct_martix(self, pred_bbox, gt_bbox):  # 计算大于IOU阈值(与gt匹配)的bbox索引
        ...
        #iou = self.cal_box_iou_t2a(gt_bbox, pred_bbox)
    ...
    def textregion_2_aligncell(self, text_region_pred, align_cell_gt, pred_logi):
        ...
        iou = self.cal_box_iou_t2a(text_region_pred, align_cell_gt)  # (N, M) float

        x = torch.where(iou >= self.iouv)  # IoU > Threshold, 返回两个tensor数组表示达到阈值的元素的横纵坐标，即pred_id, gt_id
        ...
        return gt_match_pred, new_pred_logi

    def aligncell_2_textregion(self, align_cell_pred, text_region_gt, gt_logi):
        ...
        iou = self.cal_box_iou_a2t(align_cell_pred, text_region_gt)  # (N, M) float
        ...
        return gt_match_pred, new_gt_logi
    ...
    def cal_bbox_detection(self, pred_bbox,
                           gt_bbox):  # for Task 1/2. Bounding Box Dection; Text Region Detection and Aligned Cell Detection (according to the dataset)
        ...
        self.BBD_allp += len(pred_bbox)

        self.BBD_allt += len(gt_bbox)

        res, _ = self.correct_martix(pred_bbox, gt_bbox)  # (Tensor[N]) bool

        tp = int(torch.count_nonzero(res))

        self.BBD_tp += tp
        print(f'@wzw test: self.BBD_tp={self.BBD_tp}, self.BBD_allp={self.BBD_allp}, self.BBD_allt={self.BBD_allt}')
    ...
    def cal_logical_location(self, pred_logi, gt_logi, gt2pred):  # For Logical Location F1
    ...
    def Cal_Logical_Location_F1(self):

        """

        Logical location P,R,F1 for Task4
    ...
    def run_one_step(self, preds, gts):  # 每次执行一个sample，累加各项指标
        ...
        if self.t2a and len(gt_logi) > 1:

            gt2pred, pred_logi = self.textregion_2_aligncell(pred_bbox, gt_bbox, pred_logi)

        elif self.a2t and len(gt_logi) > 1:

            gt2pred, gt_logi = self.aligncell_2_textregion(pred_bbox, gt_bbox, gt_logi)

        else:

            _, gt2pred = self.correct_martix(pred_bbox, gt_bbox)

        if ('ttd' in self.cal_tasks or 'acd' in self.cal_tasks) and (not self.t2a or not self.a2t):
            
            self.cal_bbox_detection(pred_bbox, gt_bbox)
    ...
    def summary_for_final_results(self):
        ...
        if ('ttd' in self.cal_tasks or 'acd' in self.cal_tasks) and (not self.t2a or not self.a2t):
            ...
            res += f'Text Region Detection/Aligned Cell Detection:   Precision:{BBD_P}, Recall:{BBD_R}, F1:{BBD_F1}\n'

        if 'ctc' in self.cal_tasks:
            CLS_F1 = self.Cal_Cell_Type_Classification_F1()

            res += f'Cell Type Classification:  F1:{CLS_F1}\n'

        if 'tsr_rc' in self.cal_tasks:
            RC_P, RC_R, RC_F1 = self.Cal_Relation_Classification_F1()

            res += f'TSR-Relationship Classification:   Precision:{RC_P}, Recall:{RC_R}, F1:{RC_F1}\n'

        if 'tsr_teds' in self.cal_tasks:
            TEDS_total = self.Cal_TEDS()

            res += f'TSR-TEDS:  Precision:, Recall:, F1:{TEDS_total}\n'

        if 'tsr_ll' in self.cal_tasks:
            LL_P, LL_R, LL_F1, location_res = self.Cal_Logical_Location_F1()

            res += f'TSR-Logical Location:  Precision:{LL_P}, Recall:{LL_R}, F1:{LL_F1}\n'
