...
sys.path.append('/fm1/wangziwei/wzw/TableMASTER-mmocr-master/')
...
def get_label(abs_path, img_name):
    ann_path = abs_path + img_name + '.json'
    bbox = []
    split = ''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir',
                        type=str, default='./data_result/tablegraph24k_res/html', help='Dir to save results')
    ...
    parser.add_argument('--data_abs_fold', type=str, default='/shared/aia/alg/xyl/tsrdataset/unify/tablegraph24k/')
    args = parser.parse_args()
    
    from EagleLab_Evaluator import Evaluator
    tasks = ['acd']
    eval_fn = Evaluator(cal_tasks=tasks, text_match_align=True, align_match_text=False)
    ...
    test_data = read_files_in_folder(abs_fold+'image/')
    ...
    for img_path in tqdm(test_data):
        img_name = get_img_name(img_path)[:-4]
        
        if not os.path.exists(abs_fold+'ann/'+img_name+'.json'): continue
        
        
        label, split = get_label(abs_path=abs_fold+'ann/', img_name=img_name)
        ...
        gts = {'gt_bbox':label, 'gt_lloc':[]}
        preds = {'pred_bbox':pred_bboxs, 'pred_lloc':[]}
