...
class PubtabnetParser(object):
    def __init__(self, img_dir, jsonl_path, save_root, is_toy=True, split='val', is_pse_preLabel=False, chunks_nums=16):
        self.split = split
        self.raw_img_root = img_dir
        self.save_root = save_root
        self.detection_txt_folder = os.path.join(self.save_root, 'TxtPreLabel_{}/'.format(split))
        self.recognition_folder = os.path.join(self.save_root,  'recognition_{}_img/'.format(split))
        self.recognition_txt_folder = os.path.join(self.save_root,  'recognition_{}_txt'.format(split))
        self.jsonl_path = jsonl_path
        # print(jsonl_path)
        self.structure_txt_folder = os.path.join(self.save_root,  'StructureLabelAddEmptyBbox_{}/'.format(split))
        self.is_toy = is_toy
        self.is_pse_preLabel = is_pse_preLabel
        #self.dataset_size = 4096 if is_toy else float("inf")
        self.dataset_size = 24 if is_toy else float("inf")
        self.chunks_nums = chunks_nums

        # alphabet path
        self.alphabet_path_1 = os.path.join(self.save_root,  'structure_alphabet.txt')
        self.alphabet_path_2 = os.path.join(self.save_root,  'textline_recognition_alphabet.txt')
    ...
    def merge_token(self, token_list):
        ...
        while token_list[pointer] != '</tbody>' and token_list[pointer] != '</table>':
            ### xyl add th
            if token_list[pointer] == '<td>' or token_list[pointer] == '<th>':
    ...
    def insert_empty_bbox_token(self, token_list, cells):
        ...
        for token in token_list:
            if token == '<td></td>' or token == '<td' or token == '<th></th>' or token == '<th':
    ...
    def count_merge_token_nums(self, token_list):
        ...
        for token in token_list:
            if token == '<td' or token == '<th':
                count += 1
            elif token == '<td></td>' or token == '<th></th>':
                count += 1
    ...
    def parse_single_chunk(self, this_chunk, chunks_idx):
        ...
        for item in tqdm(self.data_generator):
            ...
            image_path = os.path.join(self.raw_img_root, filename)
            ...
            # encoded_token = self.insert_empty_bbox_token(merged_token, cells)
            encoded_token = merged_token
            if len(encoded_token) > self.max_seq_len:
                self.max_seq_len = len(encoded_token)
            ...
            assert cell_nums == cell_count, print(cell_nums, cell_count, len(encoded_token))
            for cell in cells:
                if 'bbox' not in cell.keys():
                    bbox_line = ','.join([str(0) for _ in range(4)]) + '\n'
                else:
                    bbox_line = ','.join([str(int(b)) for b in cell['bbox']]) + '\n'
                structure_fid.write(bbox_line)

            # if need pse preLabel, this part will get cell coord txt files.
            # if self.is_pse_preLabel:
            #     #    pre_label_filename = 'gt_' + filename.replace('.jpg', '.txt')
            #     pre_label_filename = 'gt_' + filename.split('.')[0] + '.txt'
            ...
            #         continue
                ...
                # content = self.remove_Bb(content)
...
if __name__ == '__main__':
    ...
    arg_parser = argparse.ArgumentParser(description='data process.')
    arg_parser.add_argument('--dataset_name', type=str, default='icdar13')
    arg_parser.add_argument('--nproc', type=int, default=16)
    arg_parser.add_argument('--split', type=str, default='train', help='train or val phase.')
    arg_parser.add_argument('--ann_base', type=str, 
                            default='/shared/aia/alg/xyl/tsrdataset/model/tablemaster')
    arg_parser.add_argument('--img_base', type=str,
                            default='/shared/aia/alg/xyl/tsrdataset/unify')
    args = arg_parser.parse_args()
    
    nproc = args.nproc
    split = args.split
    dataset_name = args.dataset_name
    img_dir = os.path.join(args.img_base, f'{dataset_name}/image')
    ann_dir = os.path.join(args.ann_base, dataset_name)
    jsonl_path = os.path.join(args.ann_base, f'{dataset_name}/{split}.jsonl')
    
    parser = PubtabnetParser(img_dir, jsonl_path, save_root=ann_dir, is_toy=False, split=split, is_pse_preLabel=False, chunks_nums=nproc)
    # multiprocessing
    # start_time = time.time()
    ...
    print("parse {} set images cost {} seconds.".format(split, time.time()-start_time))

    # get structure recognition alphabet.
    parser.get_structure_alphabet()
    
    print(f'{split} max seq len: {parser.max_seq_len}')
